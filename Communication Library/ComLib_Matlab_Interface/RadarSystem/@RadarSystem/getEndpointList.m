function [EndpointTypes, EndpointTags, EndpointVersions] = getEndpointList(obj)
...
try
    ...
    if ((RX_startbyte == obj.cStartbyteMessage) && (RX_endpointID == 0))
        ...
        if ((RX_payload(1) ~= 0) && (RX_payloadSize == 2 + 6 * numEndpoints))
            disp('[RadarSystem.getEndpointTags] Error: Could not get endpoint table!')
            return;
        end
    ...
    disp(['[RadarSystem.getEndpointTags] Error: Unable to write to USB! Exception: ', exeption.message])
end
